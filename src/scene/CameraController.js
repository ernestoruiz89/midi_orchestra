...
export class CameraController {
  constructor(camera, domElement, scene) {
    ...
    this.presets = {
      ...
      acousticGuitar: { pos: new THREE.Vector3(1.05, 1.50, 2.55), target: new THREE.Vector3(1.05, 1.15, 1.42) },
      acousticGuitar_2: { pos: new THREE.Vector3(1.30, 1.42, 2.30), target: new THREE.Vector3(1.28, 1.08, 1.20) },
      ...
      cello: { pos: new THREE.Vector3(-2.85, 1.60, 1.85), target: new THREE.Vector3(-3.30, 0.90, -1.20) },
      flute: { pos: new THREE.Vector3(1.15, 1.56, 2.22), target: new THREE.Vector3(1.60, 1.30, 1.80) },
      ...
      trumpet_2: { pos: new THREE.Vector3(4.50, 1.56, 1.75), target: new THREE.Vector3(4.85, 1.40, 0.75) },
      sax_2: { pos: new THREE.Vector3(2.55, 1.55, 2.85), target: new THREE.Vector3(3.85, 1.25, 2.35) },
      violin_2: { pos: new THREE.Vector3(-4.0, 1.80, -1.1), target: new THREE.Vector3(-5.1, 1.30, -2.6) },
      cello_2: { pos: new THREE.Vector3(-3.25, 1.35, -0.05), target: new THREE.Vector3(-3.70, 1.00, -1.60) },
  ...
  setActiveInstruments(activeSet) {
    ...
    if (this.currentPreset !== 'overview' && this.currentPreset !== 'conductor') {
      ...
      const isVisible = this.activeInstruments && (
        ...
        (this.currentPreset === 'guitar_neck' && this.activeInstruments.has('guitar'))
      );
    ...
    if (this.directorMode && window.app?.midiPlayer?.isPlaying && !this.isTransitioning) {
      ...
      if (shouldSwitch) {
        ...
        if (topInst) {
          ...
          if (topInst === 'acousticGuitar') {
            nextPreset = 'acousticGuitar';
          } else if (topInst === 'guitar') {
